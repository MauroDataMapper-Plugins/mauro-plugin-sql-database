...
@Command(name = 'mauro-plugin-database-sqlserver', description = 'Extract metadata from SQLServer schema',
    mixinStandardHelpOptions = true, showAtFileInUsageHelp = true)
@Slf4j
@CompileStatic
class SQLServerCommand implements Runnable {
    ...
    static void main(final String[] args) throws Throwable {
        new CommandLine(new SQLServerCommand()).execute(args)
    }

    void run() {

        final SQLServerImportParams serverImportParams = new SQLServerImportParams(
        ...
        try{
            ...
            SQLServerDatabaseDomainDataModelImporter serverDataModelImporter = applicationContext.getBean(SQLServerDatabaseDomainDataModelImporter)
